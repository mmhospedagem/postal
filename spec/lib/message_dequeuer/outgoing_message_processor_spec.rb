...
module MessageDequeuer
  ...
  RSpec.describe OutgoingMessageProcessor do
    ...
    context "when the message already has an x-mvo-msgid header" do
      let(:message) do
        MessageFactory.outgoing(server, domain: domain, credential: credential) do |_, mail|
          mail["x-mvo-msgid"] = "existing-id"
        end
      end

      it "does not another one" do
        processor.process
        expect(message.reload.headers["x-mvo-msgid"]).to eq ["existing-id"]
      end
    ...
    context "when the message does not have a x-mvo-msgid header" do
      it "adds it" do
        processor.process
        expect(message.reload.headers["x-mvo-msgid"]).to match [match(/[a-zA-Z0-9]{12}/)]
      end
